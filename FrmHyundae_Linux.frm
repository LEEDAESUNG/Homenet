...
Begin VB.Form FrmHyundae_Linux 
   BorderStyle     =   1  '단일 고정
   Caption         =   "현대통신 홈넷 프로그램(리눅스)"
   ClientHeight    =   6990
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   17430
   ...
   End
End
Attribute VB_Name = "FrmHyundae_Linux"
...
Private Sub cmd_Test_Click()
    If Len(txt_Dong.Text) <> 0 And Len(txt_Ho.Text) <> 0 Then
        'Call HyunDaeCom_Proc(Trim(txt_Dong.Text), Trim(txt_Ho.Text), "01가1234")
        Call HyunDaeCom_Proc_Linux(Trim(txt_Dong.Text), Trim(txt_Ho.Text), "01가1234")
    Else
...
Call HomeLogger("[HomeNet Program ] Hyundae HomeNet Start..!!")
...
On Error GoTo Err_P
    ...
    '현대통신 세대통보
    Call HyunDaeCom_Proc_Linux(HomeNet_Dong, HomeNet_Ho, HomeNet_CarNo)
...
Public Sub HyunDaeCom_Proc_Linux(Dong As String, Ho As String, CarNo As String)
Dim Socket_Data_len As String * 8

On Error GoTo Err_Proc

Socket_Data = "Type=PARKING&Dong=" & Val(Dong) & "&Ho=" & Val(Ho) & "&CarNo=" & Right(Trim(CarNo), 4) & "&DateTime=" & Format(Now, "yyyymmddhhnn") & "&InOut=IN"
RSet Socket_Data_len = Len(Socket_Data)
Socket_Data_len = Replace(Socket_Data_len, " ", "0")
Socket_Data = Socket_Data_len & Socket_Data
Call HomeLogger("[HyunDaeCom_Proc_Linux]  SND : " & Socket_Data)
...
Err_Proc:
    Call HomeLogger("[HyunDaeCom_Proc_Linux]  " & Err.Description)
End Sub
...
On Error GoTo Err_P
    ...
    '현대정보통신 세대통보
    'List1.AddItem Format(Now, "yyyy-mm-dd hh:nn:ss") & " [단지서버접속 성공] IP : " & HomeSvr_IP & " Port = " & HomeSvr_Port, 0
    sData = Socket_Data
    ReDim bdata(Len(sData) - 1) As Byte
    bdata = StrConv(sData, vbFromUnicode)
    'List1.AddItem Format(Now, "yyyy-mm-dd hh:nn:ss") & " [단지서버패킷 전송] IP : " & HomeSvr_IP & " Port = " & HomeSvr_Port, 0
...
Call HomeLogger("[HyunDaeCom_Proc_Linux]  RCV : " & rMsg)
...
Err_P:
    Call HomeLogger(" [HyunDaeCom_Proc_Linux RCV] 에러내용 : " & Err.Description)
End Sub
